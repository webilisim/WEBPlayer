...
/**
 * Created by Webilisim.net On 24/12/2018
 */
public class WEBPlayerTexture extends TextureView {
    protected static final String TAG = "ResizeTextureView";

    public int currentVideoWidth = 0;
    public int currentVideoHeight = 0;

    public WEBPlayerTexture(Context context) {
        super(context);
        currentVideoWidth = 0;
        currentVideoHeight = 0;
    }

    public WEBPlayerTexture(Context context, AttributeSet attrs) {
        super(context, attrs);
        currentVideoWidth = 0;
        currentVideoHeight = 0;
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (parentWidth != 0 && parentHeight != 0 && videoWidth != 0 && videoHeight != 0) {
            if (WEBPlayer.VIDEO_IMAGE_DISPLAY_TYPE == WEBPlayer.VIDEO_IMAGE_DISPLAY_TYPE_FILL_PARENT) {
        ...
        if (parentWidth != 0 && parentHeight != 0 && videoWidth != 0 && videoHeight != 0) {
            if (WEBPlayer.VIDEO_IMAGE_DISPLAY_TYPE == WEBPlayer.VIDEO_IMAGE_DISPLAY_TYPE_ORIGINAL) {
                height = videoHeight;
                width = videoWidth;
            } else if (WEBPlayer.VIDEO_IMAGE_DISPLAY_TYPE == WEBPlayer.VIDEO_IMAGE_DISPLAY_TYPE_FILL_SCROP) {
                if (viewRotation == 90 || viewRotation == 270) {
                    int tempSize = parentWidth;
                    parentWidth = parentHeight;
                    parentHeight = tempSize;
